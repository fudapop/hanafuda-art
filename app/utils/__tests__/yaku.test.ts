...
describe('YAKU', () => {
  ...
  it('correctly calculates extra points', () => {
    const collectedExtra: Set<CardName> = new Set([...kasu, 'botan-no-kasu-1', 'botan-no-kasu-2'])
  ...
  it('correctly upgrades 3/4/5 brights', () => {
    const brights: Set<CardName> = new Set(['matsu-ni-tsuru', 'sakura-ni-maku', 'susuki-ni-tsuki'])
...
describe('checkAll', () => {
  ...
  it('returns info for completed yaku', () => {
    ...
    ;['matsu-ni-tsuru', 'sakura-ni-maku', 'susuki-ni-tsuki'].forEach((card) =>
      collected.add(card as CardName),
    )
    completed.push('sankou')
    const newReport = getCompleted(collected, completed)
    expect(newReport).toHaveLength(2)
    expect(newReport[1].cards).toEqual(['matsu-ni-tsuru', 'sakura-ni-maku', 'susuki-ni-tsuki'])
  })
})
