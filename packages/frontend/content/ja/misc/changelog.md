...
### 新機能
- **リアルタイムマルチプレイヤー（オープンベータ）**: 招待コードで友達とライブこいこい対戦ができます。共有コードでゲームを作成または参加でき、切断時も自動保存で再開可能です。
- **試合後の結果ページ**: 各ゲーム後に詳細な統計を表示する新しいマッチ統計画面。
- **マルチプレイヤーフィードバックフォーム**: 各マルチプレイヤーマッチ後に体験を共有し、問題を報告できます（Firestoreバックアップ付き）。
- **フォーフィットとアクセシビリティ**: マルチプレイヤーマッチの降参オプション、およびマルチプレイヤーUI全体のアクセシビリティ改善。
- **個別のマッチルール**: マルチプレイヤーゲームに独自のルール設定が追加され、シングルプレイヤーのUI設定とは独立しています。
- **お知らせバナー**: お知らせモーダルを、明示的に閉じるまで表示される控えめな通知バナーに置き換えました。

### パフォーマンスと技術的改善
- **Lighthouseスコアの改善**: CLS、アクセシビリティ、パフォーマンスのスコアを改善。
- **モノレポへの再構築**: プロジェクトをpnpmモノレポに再構築し、組織化を改善。

## [2026.2.0] - 2026-02-20

### バグ修正
- **匿名プレイヤーのデッキデザイン選択**: 匿名（未認証）プレイヤーがアカウントが必要なカードデッキデザインを選択できてしまう問題を修正しました。ロックされたデザインはサインイン済みのプレイヤーのみに適切に制限されるようになりました。

## [2025.12.0] - 2025-12-07

### UIと体験の改善
- **ラウンド結果の視認性**: ラウンド結果画面のテキストの視認性を向上し、読みやすさを改善しました。

### バグ修正
- **手札操作のガード**: プレイヤー選択フェーズ以外での手札の操作を防ぐための追加ガードを設けました。これにより、ゲームアクションが正しいタイミングのみで可能になります。
