...
export const useCardDesign = () => {
  ...
  const getCardMap = async (cardDesign: CardDesign) => {
    ...
    const urls = await Promise.all(
      DECK.map(
        async (cardName) =>
          await getImage(`cards/${cardDesign}/${cardName}.${getDesignInfo(cardDesign).formatExt || "webp"}`),
      ),
    );
  ...
  const useDesignPath = computed(() => (cardName: CardName) => `cards/${useDesign().value}/${cardName}.webp`);
