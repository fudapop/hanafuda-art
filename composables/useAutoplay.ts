...
export const useAutoplay = () => {
	...
	const { select, matchOrDiscard, draw, collect } =
		useCardHandler().useActions();
		...
		errorOnTimeout(
			async () => {
				...
				if (ds.getCurrent.player === "p2") {
					...
					console.warn(
						"Function 'opponentPlay' called while Player 1 is active."
					);
				}
			},
			10000,
	...
	const autoDecision = async () => {
		...
		const koikoiCalled =
			cs.handNotEmpty("p2") &&
			Math.floor(Math.random() * 100) < callChance;
		while (decisionIsPending.value) {
			await sleep(2000);
			if (koikoiCalled) {
				callKoikoi();
	...
	const calcKoikoiChance = () => {
		...
		percentChance +=
			cs.hand[player].size < 2 ? -75 : 5 * cs.hand[player].size;
