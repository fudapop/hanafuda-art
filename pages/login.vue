<template>
  <div class="min-h-screen">
    ...
    >
      <div v-if="loggingIn" class="absolute inset-0 h-full mx-auto pointer-events-none top-1/3 w-max isolate">
        <SakuraLoader class="scale-[2] ml-4 origin-bottom opacity-80" />
        <div class="font-semibold tracking-wide text-center text-gray-900 animate-pulse">Just a moment...</div>
      </div>
      <div
        v-else
        ...
        <div class="max-w-sm mx-auto min-w-[300px] w-[90vw] lg:w-96">
          <div class="relative my-auto drop-shadow-xl">
            <h1 id="login-title" class="text-4xl text-center">
              <span class="block ml-16 text-xl italic text-left text-gray-700 dark:text-white">Hanafuda</span>花札
              KOI-KOI
            </h1>
          </div>

          <div class="mt-10 space-y-5">
            <EmailLoginForm @success="handleLoginSuccess" @linked="handleLinked" @error="handleLoginError" />

            <OAuthSignupForm v-if="currentUser" @success="handleLinked" @error="handleLoginError" />
            <OAuthLoginForm v-else @success="handleLoginSuccess" @error="handleLoginError" />

            <div class="w-full text-sm text-center">
              <NuxtLink :external="!currentUser" to="/" class="text-indigo-500 hover:text-indigo-400">
                Continue as guest
              </NuxtLink>
            </div>
          </div>
