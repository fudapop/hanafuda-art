<template>
  <div ref="settingsPanel" class="grid max-h-[calc(100dvh-84px)] [@media(max-height:500px)]:max-h-[100dvh] xs:max-h-[75vh] gap-4 p-4 overflow-y-auto">
    <ClientOnly>
      ...
      <div class="mb-4 space-y-4">
        ...
        <ToggleSwitch :callback="toggleDouble" :init-value="config.doubleScoreOverSeven">
          ...
          <template #description
            >Double the score if the combined yaku value is greater than 7
            points.</template
          >
        </ToggleSwitch>
      </div>
...
const saveSettings = () => {
  ...
  if (user.settings) {
    console.info("Updating settings...", user.settings);
